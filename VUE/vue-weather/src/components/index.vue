<template>
  <div class="contaner">
    ...
    <div class="select-city-box" v-show="cityBox">
        <van-area :area-list="areaList" :columns-num="2" title="选择城市" @cancel="hide" @confirm="confirm"/>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
      ...
      AMap.plugin("AMap.Weather", function() {
        ...
        weather.getForecast(cityName, function(err, data) {
            ...
            _self.futureTem.map((item, index) => {
                _self.seriesData.push(item.dayTemp) //将天气预报的数据push到seriesData中
            })
            _self.loader = false
            _self.initEchart()
        })
    ...
    confirm(res) {
      ...
      this.seriesData = []
      this.getCurrentCityData(res[1].name)
      this.hide()      
    }
  }
}
